...
main_data = "data_2epoch"
annot_clusters = "data_2epoch/annot_clust_temp.csv"
...
def predict(text, pipeline):
    ...
    model, umap = load_models_ST(pipeline)
    ...
    if pipeline in ['paraphrase_UMAP_HDBSCAN', 'paraphrase_LoRA_UMAP_HDBSCAN']:
        ...
        return assign_cluster_from_knn(emb, emb_corpus, labels)
...
with tab3:
    ...
    # =====================================================
    # 🔵 SUBTAB 1 = MODELISATION (Intelligence Artificielle)
    # =====================================================
    with subtab1:
    ...
    # =====================================================
    # 🟢 SUBTAB 2 = DEMO DATA (Exploration des 40 000 avis)
    # =====================================================
    with subtab2:
...
with tab4:
    ...
    with subtab2:
        ...
        if option == 'paraphrase_UMAP_HDBSCAN':
            ...
            df_clust = pd.read_csv(annot_clusters)
            df_clust = df_clust.loc[df_clust['pipeline'] == 1]
            df_clust = df_clust.drop('pipeline', axis=1)
            df_clust.set_index('cluster', inplace=True)
            st.write(df_clust)
            ...
            df_clust = pd.read_csv(annot_clusters)
            df_clust = df_clust.loc[df_clust['pipeline'] == 2]
            df_clust = df_clust.drop('pipeline', axis=1)
            df_clust.set_index('cluster', inplace=True)
            st.write(df_clust)
            ...
            df_clust = pd.read_csv(annot_clusters)
            df_clust = df_clust.loc[df_clust['pipeline'] == 5]
            df_clust = df_clust.drop('pipeline', axis=1)
            df_clust.set_index('cluster', inplace=True)
            st.write(df_clust)
            ...
            df_clust = pd.read_csv(annot_clusters)
            df_clust = df_clust.loc[df_clust['pipeline'] == 6]
            df_clust = df_clust.drop('pipeline', axis=1)
            df_clust.set_index('cluster', inplace=True)
            st.write(df_clust)
